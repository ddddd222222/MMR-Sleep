...
def main():
    ...
    parser.add_argument("--data_dir", type=str, default="E:\slepp\sleepdata\sleep-edf-database-expanded-1.0.0\sleep-cassette",
    ...
    for i in range(len(psg_fnames)):
        ...
        if len(move_idx) > 0 or len(unk_idx) > 0:
            ...
            select_idx = np.setdiff1d(np.arange(x.shape[1]), remove_idx)
            x = x[:,select_idx]
            y = y[select_idx]
